"""UNet skip-connection model for image painting.

This module imports torch and torch.nn locally to ensure symbols are defined.
"""
try:
    import torch
    import torch.nn as nn
except Exception:  # pragma: no cover - environments without torch
    # define lightweight stubs so this module can be imported in non-ML envs
    class _Stub:
        def __getattr__(self, name):
            raise RuntimeError("Torch is required to use the model")
...
if __name__ == "__main__":
    try:
        ...
        print("Failed to instantiate model (torch may be missing):", e)
